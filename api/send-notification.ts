...
if (!admin.apps.length) {
  if (!process.env.FIREBASE_PROJECT_ID || !process.env.FIREBASE_CLIENT_EMAIL || !process.env.FIREBASE_PRIVATE_KEY) {
    console.error("❌ Firebase Admin env variables manquantes !");
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // =====================================================
    // 🔹 Récupération tokens FCM depuis Supabase
    // =====================================================
    ...
    if (!tokens.length) {
      console.log("⚠️ Aucun token FCM trouvé pour ce sellerId");
    ...
    console.log("📌 Message FCM ready to send:", message);

    // =====================================================
    // 🚀 Envoi via Firebase Admin
    // =====================================================
    let response;
    try {
